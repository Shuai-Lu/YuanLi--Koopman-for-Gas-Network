function EDMD = func_multi_order_EDMD_modified(x, u, y, model_order)
%%
...
EDMD.var.B11 = sdpvar(num_x, num_u, model_order, 'full');
EDMD.var.A11_x = sdpvar(num_sample - model_order, num_x, model_order, 'full');
EDMD.var.B11_u = sdpvar(num_sample - model_order, num_x, model_order, 'full');
...
EDMD.var.B21 = sdpvar(num_y, num_u, model_order, 'full');
EDMD.var.B22 = sdpvar(num_y, num_u, model_order, 'full');
...
EDMD.var.B21_u = sdpvar(num_sample - model_order, num_y, model_order, 'full');
EDMD.var.B22_du = sdpvar(num_sample - model_order, num_y, model_order, 'full');
EDMD.var.error_y = sdpvar(num_sample - model_order + 1, num_y, 'full');

%% define cons
% y(k+1) = sum(A21(i)*x(k-i)) + sum(A22(i)*y(k-i)) + 
%          sum(B21(i)*u(k-i)) + sum(B22(i)*(u(k-i)-u(k-i-1)))
k_interval = 2 : num_sample - model_order;
for k_order = 1 : model_order
    EDMD.cons = EDMD.cons + (( ...
    ...
    EDMD.cons = EDMD.cons + (( ...
        EDMD.var.B22_du(k_interval, :, k_order)' ==  ...
...
EDMD.cons = EDMD.cons + (( ...
    ...
    sum(EDMD.var.B22_du(k_interval,:,:), 3)...
