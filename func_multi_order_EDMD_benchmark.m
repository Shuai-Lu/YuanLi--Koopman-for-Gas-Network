...
if  ~ isempty(x)
    ...
    EDMD.var.S = sdpvar(num_x, num_x, model_order_x + 1);
    ...
    EDMD.var.sys_matrix = [reshape(EDMD.var.A(:,:,2:end), num_x, []); 
        ones(num_x*model_order_x, num_x*(model_order_x-1)) zeros(num_x*model_order_x, num_x)];
    EDMD.cons = EDMD.cons + ((norm(EDMD.var.sys_matrix,2) <= 1 - 1e-2));  % 0.99
